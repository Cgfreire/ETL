...
def carga_full():
  ...
  for i in range(0,len(FactIndicadores)):
    sql_insert_dim_factIndicadores(FactIndicadores['PRECO_ABER'].iloc[i],FactIndicadores['PRECO_MAX'].iloc[i],FactIndicadores['PRECO_MIN'].iloc[i]
...
with DAG(dag_id ='carga_full_b3',start_date=datetime.datetime(2024, 10, 31), schedule="@hourly") as dag:
